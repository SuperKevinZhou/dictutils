...
fn test_mdx_encoding_fix() {
    println!("Testing MDX encoding fix...");
...
#[test]
fn test_encoding_conversion_function() {
    println!("Testing encoding conversion function...");

    // Test data that represents GBK-encoded Chinese text
    let test_data = vec![
        0xD6, 0xD0, 0xB9, 0xFA, 0xC8, 0xCB, 0xC2, 0xDB, 0xCA, 0xC7, // "测试数据" in GBK
    ];

    // Verify the test data is NOT valid UTF-8
